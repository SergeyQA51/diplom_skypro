...
def test_catalog_search():
    with allure.step("Открытие веб-страницы в Chrome и выполнение поиска"):
        ...
        main_page = MainPage(browser) 
        main_page.set_cookie_policy()
        text = main_page.catalog_search()  # Предполагается, что метод возвращает текст первого результата.

    with allure.step("Проверка наличие класса 'products-list' на странице"):
        products_list = browser.find_elements(By.CLASS_NAME, "products-list")  # Получаем элементы с классом 'products-list'
        
        # Проверяем, что список продуктов не пуст
        assert len(products_list) > 0, "На странице отсутствуют элементы с классом 'products-list'."

    with allure.step("Проверка текста в выбранной категории каталога"):
        assert text[0:14] == "ПОЭЗИЯ И СТИХИ", "Текст первого результата не соответствует ожиданиям."  
